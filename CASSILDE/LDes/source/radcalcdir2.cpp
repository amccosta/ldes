#include "sampling/ldradparser.h"
#include "sampling/qhsarea.h"
...
#include "mesh.h"
#define MESH_SIZE_MAX (2*MESH_SIZE)
...
#endif
#else
#define TEMP1 "/usr/tmp/rt.in"
#define TEMP2 "/usr/tmp/rt.ou"
#define PNAME "/usr/tmp/rt.%s.pf"
...
static data_struct data1[MESH_SIZE_MAX+1],data2[MESH_SIZE_MAX+1];
...
typedef struct _dist_struct {
  ...
  float l[MESH_SIZE_MAX];
  float dx[MESH_SIZE_MAX];
  float dy[MESH_SIZE_MAX];
  float dz[MESH_SIZE_MAX];
} dist_struct;
static dist_struct point[POINT_MAX];
static int point_ptr=-1;

static float dist_thres=DIST_THRES;
static char radcalc_init=1;
...
static int
list_cmp(const void *a,const void *b)
{
  query_struct *P1 = (query_struct *)a;
  query_struct *P2 = (query_struct *)b;
...
init_point()
{
  ...
  for (i=0; i<POINT_MAX; i++)
  {
    ...
    point[i].n=MESH_SIZE_MAX;
    for (j=0; j<MESH_SIZE_MAX; j++)
      point[i].l[j]=-1;
  }
  point_ptr=0;
}

static float dweight[MESH_SIZE_MAX];
static float dpoint[3*MESH_SIZE_MAX];

static void
init_dirs(float P1,float P2,float P3,float P4,float P5,float P6,
      float P7,data_struct *P8)
{
  float vx=-P4;
  float vy=-P5;
  float vz=-P6;
  ...
  for (i=mesh_size; i<MESH_SIZE_MAX; i++) P8[i].v=CODE_IGNORED;
  if (objList.GetSize()<1) return;

  n=FindNewDirs(P1,P2,P3,P4,P5,P6,P7,MESH_SIZE_MAX-mesh_size,
                &objList,dpoint);

#if 1
  fprintf(stderr, "FND: %d\n", n);
#endif
...
#if 0
  {
  FILE *ou=fopen("spoints.txt","w");
    if (ou)
    {
      fprintf(ou,"3\n%d\n",mesh_size);
      for (i=0; i<mesh_size; i++)
        fprintf(ou,"%g %g %g\n",P8[i].x,P8[i].y,P8[i].z);
      if (ou) fclose(ou);
    }
  }
#endif

  if (GetPointAreas(mesh_size,dpoint,dweight))
  {
    for (i=0; i<mesh_size; i++) P8[i].w=dweight[i];
    return;
  }

  /* error in GetPointAreas - revert to standard sampling */

#if 0
  fprintf(stderr,"GPA: returned 0\n");
#endif
...
{
  char fname[STR_SIZE];
  FILE *in=fopen("rads.lst","r");
  if (!in) return;
  while (!feof(in) && !ferror(in))
  {
    if (!fscanf(in,"%s",fname) || !strcmp(fname,"END"))
    {
      fclose(in);
      return;
    }
    ParseRADFile(fname,&matList,&objList);
  }
  fclose(in);
}
           ...
           char *P9,float P10,float P11,float P12)
{
  ...
  init_dirs(P2,P3,P4,P5,P6,P7,P8,data2);
  for (i=0; i<npoint; i++)
  {
    int m;
...
main(int argc,char *argv[])
{
  ...
  if (argc==8)
  {
    ...
    fprintf(stderr,"%s %g %g %g %g %g %g %g: %g\n",
      argv[1],P2,P3,P4,P5,P6,P7,P8,
      radcalcdir(argv[1],P2,P3,P4,P5,P6,P7,P8,"",0.0,0.0,0.0));
    return 0;
  }
  do {
    ...
    fprintf(stderr,"%s %g %g %g %g %g %g %g: %g\n",P1,P2,P3,P4,P5,P6,P7,P8,
      radcalcdir(P1,P2,P3,P4,P5,P6,P7,P8,"",0.0,0.0,0.0));
  } while (strcmp(P1,"-"));
  return 0;
}
#endif
